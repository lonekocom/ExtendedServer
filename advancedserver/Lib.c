...
bool allocate_server(uint16_t base_port, uint16_t n)
{
    ...
    RAssert(dylist_create(&server->peers, g_config.server_config.pairing.maximum_players_per_lobby));
...
bool disaster_init(void)
{
	...
	for (int32_t i = 0; i < g_config.server_config.networking.server_count; i++)
		RAssert(allocate_server((uint16_t)g_config.server_config.networking.port, i));
