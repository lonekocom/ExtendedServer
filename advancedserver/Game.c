...
bool game_init(int exe, int8_t map, Server* server)
{
    ...
    if (!dylist_create(&server->game.left, g_config.server_config.pairing.maximum_players_per_lobby))
		return false;
...
{
    if (show_results && g_config.states.results_misc.enabled)
...
bool game_checkstart(Server* server)
{
	...
	if (cnt >= server_ingame(server))
	{
		...
		if (server->game.time_sec > g_config.states.gameplay.gametimers_ceiling && !g_config.states.gameplay.banana.disable_timer)
			server->game.time_sec = g_config.states.gameplay.gametimers_ceiling;
...
bool game_demonize(Server* server, PeerData* data)
{
    ...
    if (players * (g_config.states.gameplay.demonization_percentage / 100.0) > demonized)
	{
...
bool game_state_handletcp(PeerData* v, Packet* packet)
{
	...
	switch (type)
	{
        ...
        {
            if(g_config.states.gameplay.enable_achievements){
                AssertOrDisconnect(v->server, v->in_game);
                server_broadcast_ex(v->server, packet, true,  v->id);
            }
            break;
        ...
        {
            if(g_config.states.gameplay.enable_sounds){
                AssertOrDisconnect(v->server, v->in_game);
                server_broadcast_ex(v->server, packet, true,  v->id);
            }
            break;
		...
		{
			AssertOrDisconnect(v->server, v->in_game);
            AssertOrDisconnect(v->server, !g_config.states.gameplay.anticheat.palette_anticheat || palette_player_validate(v, packet));
			server_broadcast_ex(v->server, packet, true,  v->id);
			break;
		}
		...
		case CLIENT_TPROJECTILE:
		{
            ...
            if(v->plr.cooldown > 0 && g_config.states.gameplay.anticheat.ability_anticheat)
			{
				char msg[256];
			...
			if (v->plr.flags & PLAYER_DEMONIZED)
			{
                ...
                if (dmg >= 60 && g_config.states.gameplay.anticheat.ability_anticheat)
				{
			...
			else
			{
                ...
                if (dmg >= 6 && g_config.states.gameplay.anticheat.ability_anticheat)
				{
                    double last = time_end(&v->plr.tails_last_proj);
					check_balls = last > 1500 && last < 12000;
					AssertOrDisconnect(v->server, check_balls);
		...
		case CLIENT_PLAYER_DEATH_STATE:
		{
			...
			if (dead)
			{
				...
				else
				{
					...
					if (g_config.states.gameplay.match_respawn_and_game_timers) {
						uint16_t time_to_sudden_death = g_config.states.gameplay.banana.disable_timer
							? g_config.states.gameplay.sudden_death_timer - v->server->game.time_sec
							: v->server->game.time_sec - g_config.states.gameplay.sudden_death_timer;

						if (time_to_sudden_death < g_config.states.gameplay.respawn_time) {
							v->plr.death_timer_sec = time_to_sudden_death;
						} else {
		...
		case CLIENT_PLAYER_DATA:
		{
            ...
            if(g_config.states.gameplay.anticheat.distance_anticheat)
			{
			...
			if (v->plr.mod_tool)
			{
				...
				else
				{
					...
					else if (dist >= 300 && g_config.states.gameplay.anticheat.data_based_anticheat)
					{
						// purpl men
...
bool game_player_tick(Server* server)
{
	...
	for(size_t i = 0; i < server->peers.capacity; i++)
	{
		...
		if (server->delta < 2.5)
		{
			...
			if (data->plr.ping_timer >= 20 * TICKSPERSEC)
			{
				...
				if (avg_ping >= g_config.server_config.pairing.ping_limit)
				{
					char msg[130];
	...
	// Start demonization
    if (!server->game.sudden_death && (server->game.time_sec <= g_config.states.gameplay.sudden_death_timer) == !g_config.states.gameplay.banana.disable_timer)
	{
	...
	for (size_t i = 0; i < server->peers.capacity; i++)
	{
		...
		if (data->plr.flags & PLAYER_DEAD && data->plr.death_timer_sec > 0)
		{
			...
			for (size_t j = 0; j < server->peers.capacity; j++)
			{
                ...
                if(!g_config.states.gameplay.exe_camp_penalty)
                    continue;
            ...
            if ((server->game.time_sec < g_config.states.gameplay.sudden_death_timer) != g_config.states.gameplay.banana.disable_timer)
			{
			...
			{
				uint16_t time_to_sudden_death = g_config.states.gameplay.banana.disable_timer
					? g_config.states.gameplay.sudden_death_timer - data->server->game.time_sec
					: data->server->game.time_sec - g_config.states.gameplay.sudden_death_timer;

				if (!exe_near || (time_to_sudden_death < data->plr.death_timer_sec && g_config.states.gameplay.match_respawn_and_game_timers))
				{
...
bool game_state_tick(Server* server)
{
	...
	if (server->game.time <= TICKSPERSEC)
	{
        ...
        g_config.states.gameplay.banana.disable_timer ? server->game.time_sec++ : server->game.time_sec--;
		server->game.time += TICKSPERSEC;

		if (server->game.time_sec % server->game.ring_coff == 0)
		{
            if(g_config.states.gameplay.entities_misc.global.rings.enabled)
		...
		if (server->game.time_sec <= 0)
            RAssert(game_end(server, g_config.states.gameplay.gmcycle.overhell ? ED_SURVWIN : ED_TIMEOVER, true));
	}
