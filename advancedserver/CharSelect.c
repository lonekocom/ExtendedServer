...
bool charselect_check_state(Server* server)
{
	...
	if (should) {
		if (g_config.states.character_selection.charselect_mod_unlocked && !g_config.states.gameplay.hide_player_characters)
			for (size_t i = 0; i < server->peers.capacity; i++)
			{
				PeerData* peer = (PeerData*)server->peers.ptr[i];
				if (!peer)
...
bool charselect_state_handle(PeerData* v, Packet* packet)
{
	...
	case CLIENT_REQUEST_EXECHARACTER:
	{
		...
		if (!g_config.states.character_selection.allow_foreign_characters) {
		...
		if (!g_config.states.gameplay.hide_player_characters) {
			PacketCreate(&pack, SERVER_LOBBY_CHARACTER_CHANGE);
		...
		Info("%s " LOG_RST "(id %d) choses [" LOG_RED "%s" LOG_RST "]!", v->nickname.value, v->id, id >= 0 && id <= EX_EXELLER ? exes[id] : "placeholder");
	...
	case CLIENT_REQUEST_CHARACTER:
	{
		...
		if (avail && !g_config.states.character_selection.charselect_mod_unlocked)
		...
		if (!g_config.states.gameplay.hide_player_characters && !g_config.states.character_selection.charselect_mod_unlocked) {
			PacketCreate(&pack, SERVER_LOBBY_CHARACTER_CHANGE);
			PacketWrite(&pack, packet_write16, v->id);
			PacketWrite(&pack, packet_write8, id + 1);
			server_broadcast(v->server, &pack, true);
...
bool charselect_init(int8_t map, Server* server)
{
	...
	if (!g_config.states.character_selection.enable) {
		for (size_t i = 0; i < server->peers.capacity; i++)
		{
			PeerData* peer = (PeerData*)server->peers.ptr[i];
			...
			if (peer->id == server->lobby.exe) {
				peer->exe_char = rand() % (EX_EXELLER + 1);
				PacketCreate(&pack, SERVER_LOBBY_EXECHARACTER_RESPONSE);
				PacketWrite(&pack, packet_write8, peer->exe_char);
				RAssert(packet_send(peer->peer, &pack, true));
			}
			else {
				peer->surv_char = rand() % (CH_SALLY + 1);
				PacketCreate(&pack, SERVER_LOBBY_CHARACTER_RESPONSE);
				PacketWrite(&pack, packet_write8, peer->surv_char + 1);
			...
			PacketCreate(&pack, SERVER_LOBBY_CHARACTER_CHANGE);
			PacketWrite(&pack, packet_write16, peer->id);
			PacketWrite(&pack, packet_write8, peer->id == server->lobby.exe ? peer->exe_char : peer->surv_char + 1);
			server_broadcast(server, &pack, true);
		}
		return game_init(server->lobby.exe, server->lobby.map, server);
	}
...
bool charselect_state_left(PeerData* v)
{
	...
	if (server_ingame(v->server) <= (1 - g_config.states.gameplay.banana.singleplayer) || v->id == v->server->lobby.exe)
