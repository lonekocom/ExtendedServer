...
void lobby_check_vote(Server* server)
{
	if (vote_check(&server->lobby.vote))
	{
		switch (server->lobby.vote.type)
		{
			...
			{
				char buffer[256];
                snprintf(buffer, 256, "map vote " CLRCODE_GRN "succeeded~ (" CLRCODE_GRN "%d ~out of %d~)", server->lobby.vote.votecnt, server->lobby.vote.votetotal);
				server_broadcast_msg(server, 0, buffer);
	...
	else
	{
        switch (server->lobby.vote.type)
		{
			...
			{
				char buffer[256];
                snprintf(buffer, 256, "map vote " CLRCODE_RED "failed~ (" CLRCODE_RED "%d ~out of %d~)", server->lobby.vote.votecnt, server->lobby.vote.votetotal);
				server_broadcast_msg(server, 0, buffer);
				break;
			}
		}
...
bool lobby_state_handle(PeerData* v, Packet* packet)
{
	...
	switch (type)
	{
		...
		case CLIENT_LOBBY_PLAYERS_REQUEST:
		{
            ...
            if(g_config.states.lobby_misc.authoritarian_mode)
		...
		case CLIENT_CHAT_MESSAGE:
		{
            ...
            if(strstr(msg.value, "52") && strcmp(g_config.states.lobby_misc.server_location, "Saint Petersburg") == 0)
            {
...
bool lobby_state_tick(Server* server)
{
	...
	if (server->lobby.prac_countdown > 0)
	{
		...
		if (server->lobby.prac_countdown <= 0)
			return charselect_init(server->lobby.voting_map, server) || lobby_init(server);
	}
    ...
    if (server->lobby.countdown_sec <= COUNTDOWN && !g_config.states.lobby_misc.authoritarian_mode)
	{
...
bool lobby_state_left(PeerData* v)
{
	...
	case ST_LOBBY:
		if (v->server->lobby.vote.ongoing
			&& v->server->lobby.vote.type == VOTE_KICK
			&& v->server->lobby.kick_target.id == v->id && g_config.states.lobby_misc.votekick.autoban_leavers
		)
		{
			Info("Kick target left during vote — vote auto-succeeds");

			char buffer[64];
			snprintf(buffer, 64, "vote kick " CLRCODE_GRN "auto-succeeded~ (" CLRCODE_GRN "target left~)");
			server_broadcast_msg(v->server, 0, buffer);

			timeout_set(v->nickname.value, v->udid.value, v->ip.value, time(NULL) + 60);
			server_disconnect_id(v->server, v->id, DR_KICKEDBYHOST, NULL);

			v->server->lobby.vote.ongoing = 0;
		}
	case ST_GAME:
	case ST_RESULTS:
		break;
