...
bool mapvote_init(Server* server)
{
	...
	MutexLock(g_config.map_list_lock);
	{
        ...
        for (int8_t i = 0; i < MAP_COUNT; i++)
        {
            if (g_config.states.map_selection.map_list[i] && (!g_config.states.map_selection.exclude_last_map || i != server->last_map))
                allowed[allowed_count++] = i;
        }

        if (!g_config.states.map_selection.enabled)
        {
...
{
    if(server_ingame(v->server) <= 1 - g_config.states.gameplay.banana.singleplayer)
